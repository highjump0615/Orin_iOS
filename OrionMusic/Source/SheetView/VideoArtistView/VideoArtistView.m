...
@synthesize likeCount, commentCount, category;
@synthesize strImage;

+ (id)videoData {
	return [[VideoData alloc] init];
}
...
+ (id)videoItemView:(VideoData*)data {
	VideoItemView* itemView = [[VideoItemView alloc] initWithVideoData:data];
	return itemView;
}

- (id)initWithVideoData:(VideoData*)data {
	...
	if(self) {
        ...
        // video imge
        NSString* strUrl = [NSString stringWithFormat:@"%@/video/image/%@", SERVER_PATH, data.strImage];
		...
		[m_btnPlay setTitle:data.url forState:UIControlStateNormal];
        
        // clock image
        UIImageView* imgcategory = [[UIImageView alloc] initWithFrame:CGRectMake(10, 85, 101, 19)];
        
        switch (data.category) {
            case 0:
                [imgcategory setImage:[UIImage imageNamed:@"newrelease.png"]];
                break;
                
            case 1:
                [imgcategory setImage:[UIImage imageNamed:@"weeklychart.png"]];
                break;
                
            case 2:
                [imgcategory setImage:[UIImage imageNamed:@"exclusive.png"]];
                break;
                
            case 3:
                [imgcategory setImage:[UIImage imageNamed:@"throwback.png"]];
                break;
        ...
        // artist label
        UILabel *labArtist = [[UILabel alloc] initWithFrame:CGRectMake(10, 109, 265, 23)];
        ...
        // title label
        m_labTitle = [[UILabel alloc] initWithFrame:CGRectMake(10, 132, 265, 23)];
        ...
        // clock image
        UIImageView* imgclock = [[UIImageView alloc] initWithFrame:CGRectMake(224, 11, 12, 12)];
		[imgclock setImage:[UIImage imageNamed:@"chart_clock.png"]];
		[self addSubview:imgclock];
        
        // date label
		m_labDate = [[UILabel alloc] initWithFrame:CGRectMake(241, 10, 79, 15)];
...
- (void)onBtnLike:(id)sender {
	NSString* videoID = ((UIButton*)sender).titleLabel.text;
	GameUtility* utils = [GameUtility sharedObject];
	NSDictionary* dic = [NSDictionary dictionaryWithObjectsAndKeys:
						 ...
						 nil];
	if (m_bLiked) {
		[utils runWebService:@"unlike_video" Param:dic Delegate:self View:nil];
		...
		[utils runWebService:@"like_video" Param:dic Delegate:self View:nil];
...
- (void)onBtnComment:(id)sender {
	...
	controller.m_nVideoID = [videoID integerValue];
	controller.m_videoItem = self;
...
- (void)onBtnShare:(id)sender {
    ...
    NSString* strUrl = [NSString stringWithFormat:@"%@/video/image/%@", SERVER_PATH, strImage];
...
- (void)onBtnPlay:(id)sender {
    ...
    controller.delegate = [SheetView sharedObject];
    
    [[SheetView sharedObject] presentViewController:controller animated:YES completion:nil];
}
...
@implementation VideoArtistView
...
- (void)loadVideo: (BOOL)bShowWaiting {
	...
	if (bShowWaiting)
		[utils runWebService:@"getvideo" Param:dic Delegate:self View:[self superview]];
	else
		[utils runWebService:@"getvideo" Param:dic Delegate:self View:nil];
}
...
- (void)createItemView:(UIViewController*)parent {
	...
	self.m_scrollView = [[UIScrollView alloc] initWithFrame:rt];
...
- (void)finishedRunningServiceDelegate:(NSDictionary *)outData {
	...
	if ([serviceName isEqualToString:@"getvideo"]) {
		NSArray* items = outData[@"info"];
        
        for(id item in items) {
			
			VideoData* data = [VideoData videoData];
			data.url = item[@"youtubelink"];
            ...
            data.category = [item[@"category"] integerValue];
            
			VideoItemView* itemView = [VideoItemView videoItemView:data];
			...
			[self.m_scrollView addSubview:itemView];
		...
		CGSize sz = self.m_scrollView.frame.size;
		...
		[self.m_scrollView setContentSize:sz];
